...
class MCTSSearch:
    ...
    def action_probs(self):
        ...
        scores = [c.total_visits / self.root.total_visits if c is not None else -np.inf for c in self.root.children]
...
class MCTSNode:
    ...
    def initialize(self):
        ...
        if game.game_ended(self.state):
            self.value = game.winning(self.state, self.search.komi) * game.turn_pm(self.state)
            self.terminal = True
        else:
            self.value, self.policy = self.search.model.forward(self.state) # TODO: Can we remove invalid moves from channel features?
            self.terminal = False
            self.children = [(MCTSNode(game.next_state(self.state, a), parent=self, search=self.search)) if self.valid_moves[a] == 1 else None for a in range(game.action_size(self.state))]
    ...
    def noisy_policy(self):
        if not self.search.noise:
            ...
            return 0.75 * self.policy \
                + 0.25 * np.random.dirichlet([0.03 * game.action_size(self.state) / (c.valid_move_count + 1) if c is not None else np.nextafter(0, 1) for c in self.children])
    ...
    def best_child(self, c_puct=1.1):
        ...
        puct = [-c.value_score() + c_puct * noisy_policy[i] * np.sqrt(self.total_visits - 1) / (1 + self.children[i].total_visits) 
