...
from yacgo.go import govars, game
...
def draw_circle(x, y, color, radius):
    ...
    pyglet.graphics.draw(
        len(verts) // 2, pyglet.gl.GL_TRIANGLE_FAN, ("v2f", verts), ("c3f", colors)
    )


def draw_command_labels(batch, window_width, window_height):
    pyglet.text.Label(
        "Pass (p) | Reset (r) | Exit (e)",
        font_name="Helvetica",
        font_size=11,
        x=20,
        y=window_height - 20,
...
def draw_info(batch, window_width, window_height, upper_grid_coord, state):
    ...
    prev_player_passed = game.prev_player_passed(state)
    game_ended = game.game_ended(state)
    info_label = "Turn: {}\nPassed: {}\nGame: {}".format(
    ...
    # Areas
    black_area, white_area = game.areas(state)
    pyglet.text.Label(
        "{}B | {}W".format(black_area, white_area),
        font_name="Helvetica",
        font_size=16,
        x=window_width / 2,
...
def draw_grid(batch, delta, board_size, lower_grid_coord, upper_grid_coord):
    ...
    for i in range(board_size):
        # horizontal
        ver_list.extend((lower_grid_coord, left_coord, upper_grid_coord, right_coord))
        # vertical
        ver_list.extend((left_coord, lower_grid_coord, right_coord, upper_grid_coord))
        ...
        pyglet.text.Label(
            ...
            font_name="Courier",
            font_size=11,
            x=lower_grid_coord - label_offset,
            y=left_coord,
            anchor_x="center",
            anchor_y="center",
            color=(0, 0, 0, 255),
        ...
        pyglet.text.Label(
            ...
            font_size=11,
            x=left_coord,
            y=lower_grid_coord - label_offset,
            anchor_x="center",
            anchor_y="center",
            color=(0, 0, 0, 255),
            batch=batch,
...
def draw_pieces(batch, lower_grid_coord, delta, piece_r, size, state):
    for i in range(size):
        for j in range(size):
            ...
            if state[0, i, j] == 1:
                draw_circle(
                    lower_grid_coord + i * delta,
                    lower_grid_coord + j * delta,
